...
class MediaExplorerTest(unittest.TestCase):
    ...
    def testPlayLocalVideo(self):
        ...
        d.sleep(3)
        d.press('left')
        assert d(resourceId='com.xiaomi.mitv.mediaexplorer:id/dev_name', text="移动存储设备").wait.exists(timeout=10000), 'enter Device list screen failed!'
        d.sleep(3)
        d.press('down')
        d.sleep(3)
        d.press('enter')
    ...
    def testPlayLocalVideoBack(self):
        """
        launch  app store and exit
        """
        for i in xrange(5):
